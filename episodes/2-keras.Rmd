...
To create a confusion matrix we will use another convenient function from sklearn called `confusion_matrix`.
...
- The second row: There are 14 Chinstrap pengunis in the test data, with 5 identified as Adelie (invalid), none are correctly recognized as Chinstrap, and 9 Chinstrap are identified as Gentoo (invalid).
- The third row: There are 25 Gentoo penguins in the test data, with 6 identified as Adelie (invalid), none being recognized as Chinstrap (invalid), and 19 Gentoo are identified as Gentoo (valid).
...
:::: solution
## Solution
The confusion matrix shows that the predictions for Adelie and Gentoo are decent, but could be improved. However, Chinstrap is not predicted ever. Why are the Chinstraps not present in the confusion matrix from prediction?
If we go back to the [**Pair Plot**](https://carpentries-incubator.github.io/deep-learning-intro/2-keras.html) in the Visualization section, we can figure it out that the biggest challenge is masking the Chinstrap penguins from the marginal distributions of the four features (bill length, bill depth, flipper length, and body mass). That is, there is no single variable that separates Chinstrap penguins from all other species. The combination of bill length and bill depth (the 2nd subfigure in the first row in the pairplot) gives a good separation of Chinstrap from Adelie and Gentoo penguins.
However, as there are limited number combinations that can be used for a clear classification of Penguins species, the constructed neural network (one dense layer and 10 neurons in this layer) might be difficult to generate a clear classification of Penguins species.
A practical workaround is to increase the number of dense layers and also the number of neutrons in each dense layers. In addition, adjusting the learning rate is also possible to achieve a high score for the prediction. You will get more info in the [**Advanced layer types**](https://carpentries-incubator.github.io/deep-learning-intro/4-advanced-layer-types.html) episode.

The training loss was very low, so from that perspective this may be surprising. But this illustrates very well why a test set is important to give a realistic evaluation when training neural networks.
...
Other options include: changing the network architecture or changing the training parameters
