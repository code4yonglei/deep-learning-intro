...
- The first row: There are 30 Adelie tested, with 22 Adelie identified as Adelie (valid), 8 Adelie identified as Gentoo  (invalid), and no Adelie identified as Chinstrap.
- The second row: There are 14 Chinstrap tested, with 5 Chinstrap identified as Adelie (invalid), no Chinstrap recognized as Chinstrap (valid), and 9 Chinstrap identified as Gentoo (invalid).
- The third row: There are 25 Gentoo tested, with 6 Gentoo identified as Adelie (invalid), 0 Gentoo recognized as Chinstrap (invalid), and 19 Gentoo identified as Gentoo (valid).
...
Why the chinstraps are not present in the the predictions in the confusion matrix?
If we go back to the [**Pair Plot**](https://carpentries-incubator.github.io/deep-learning-intro/2-keras.html#pair-plot) in the Visualization section, we can figure it out that the biggest challenge is masking the Chinstrap penguins from the marginal distributions of the four features (bill length, bill depth, flipper length, and body mass). That is, There is no single variable that separates Chinstrap from all other species. The combination of bill_length_mm vs. bill_depth_mm (the 2nd subfigure in the first row in the pairplot) gives a good separation of Chinstrap from Adelie and Gentoo.
